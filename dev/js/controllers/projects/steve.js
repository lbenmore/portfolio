core.controllers.Steve = () => {
  const
  responsify = () => {
    let
    gameW = parseInt(getComputedStyle(document.querySelector('.game')).width),
    gameH = parseInt(getComputedStyle(document.querySelector('.game')).height),
    gameR = gameW / gameH,
    winW = innerWidth,
    winH = innerHeight,
    winR = winW / winH;
    ...
    if (gameW > winW || gameH > winH) {
      if (gameR > winR) {
        document.querySelector('.game').style.transform = `translate(-50%, -50%) scale(${winW / gameW})`;
      } else {
        document.querySelector('.game').style.transform = `translate(-50%, -50%) scale(${winH / gameH})`;
      }
    } else {
  ...
  class Steve {
    ...
    initScore () {
      this.score = new Score(document.querySelector('.game'), 0);
    }
    ...
    initCacti (target) {
      ...
      setTimeout( () => {
        new Cactus(target, this, this.cacti, this.score);
        // setTimeout( () => {
        //   new Cactus(document.querySelector('.game'), this, this.cacti, this.score);
        // }, 1000);
      }, 2000);
    }
